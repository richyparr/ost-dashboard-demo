...
const MyVAsPage = () => (
  <div className="content-area">
    ...
    <div className="stats-grid" style={{ marginBottom: 24 }}>
      {[
        { label: 'Team Members', value: '3', change: 'All active', positive: true, icon: 'vas' },
        { label: 'Total Hours', value: '113h', change: 'This week', positive: true, icon: 'hours' },
        { label: 'Tasks Assigned', value: '18', change: '+5 this week', positive: true, icon: 'tasks' },
        { label: 'Avg Rating', value: '4.9', change: 'Excellent', positive: true, icon: 'rating' },
      ].map((stat, i) => (
        <div key={i} className="stat-card">
          <div className="stat-card-header">
            <span className="stat-label">{stat.label}</span>
            <div className={`stat-icon ${stat.icon === 'rating' ? 'warning' : 'primary'}`}>
              {stat.icon === 'vas' && <Icons.MyVAs />}
              {stat.icon === 'hours' && <Icons.Clock />}
              {stat.icon === 'tasks' && <Icons.Tasks />}
              {stat.icon === 'rating' && <Icons.Heart />}
            </div>
          </div>
          <div className="stat-value">{stat.value}</div>
          <div className={`stat-change ${stat.positive ? 'positive' : 'neutral'}`}>
            {stat.positive && <Icons.TrendUp />}
            <span>{stat.change}</span>
          </div>
        </div>
      ))}
    ...
    <div className="dashboard-bottom" style={{ marginTop: 24 }}>
      <div className="card">
        ...
        <div className="card-body">
          {[
            { va: 'Maria Santos', action: 'Completed task "Update client presentation"', time: '2 hours ago' },
            { va: 'James Chen', action: 'Started working on "Social media report"', time: '3 hours ago' },
            { va: 'Sofia Rodriguez', action: 'Resolved 5 support tickets', time: '4 hours ago' },
          ].map((activity, i) => (
            <div key={i} className="deadline-item">
              <div className="deadline-indicator medium"></div>
              <div className="deadline-info">
                <span className="deadline-title">{activity.va}</span>
                <span className="deadline-assignee">{activity.action}</span>
              </div>
              <span className="deadline-date">{activity.time}</span>
            </div>
          ))}
        </div>
      </div>
          ...
          {mockVAs.map((va) => (
            <div key={va.id} className="va-progress-item">
              ...
              <div className="va-progress-info">
                ...
                <span className="va-progress-completed">{Math.floor(Math.random() * 5) + 8} tasks completed</span>
              </div>
              <span className="va-progress-percent">{va.hoursThisWeek}h</span>
            </div>
          ))}
        </div>
      </div>
